...
class Chatbot:
    ...
    def run(self):
        assert self.args['checkpoint'], '모델의 가중치를 찾을 수 없습니다. --checkpoint CHECKPOINT_PATH 이용해보세요. '
        self._load_checkpoint()

        print('대화를 시작하겠습니다. ')
        print(f' \"{self.args["stop_command"]}\"을 입력하시면 대화가 종료됩니다. ')

        self.model.eval()

        with torch.no_grad():
            input_hists = []
            # 페르소나를 넣고 싶다면, 해당 코드를 활성화하면 됩니다. 
            # history = input('Persona: ')
            # input_ids = [self.args['sp2_id']] + self.tokenizer.encode(history)
